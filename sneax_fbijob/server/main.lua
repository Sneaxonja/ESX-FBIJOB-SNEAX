...
RegisterNetEvent('esx_fbijob:confiscatePlayerItem')
AddEventHandler('esx_fbijob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'fbi' then
		print(('esx_fbijob: %s attempted to confiscate!'):format(sourceXPlayer.identifier))
		return
	end
...
RegisterNetEvent('esx_fbijob:getStockItem')
...
RegisterNetEvent('esx_fbijob:putStockItems')
...
ESX.RegisterServerCallback('esx_fbijob:getOtherPlayerData', function(source, cb, target, notify)
...
ESX.RegisterServerCallback('esx_fbijob:storeNearbyVehicle', function(source, cb, plates)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('esx_fbijob:spawned')
...
RegisterNetEvent('esx_fbijob:forceBlip')
AddEventHandler('esx_fbijob:forceBlip', function()
	for _, xPlayer in pairs(ESX.GetExtendedPlayers('job', 'fbi')) do
		TriggerClientEvent('esx_fbijob:updateBlip', xPlayer.source)
	end
end)
